...
//
//
/// \file BoundaryEnhancementVolumeStyle_FragmentShader.glsl
/// \brief shader file for BoundaryEnhancementVolumeStyle, not used by default.
///
//
//
//////////////////////////////////////////////////////////////////////////////
...
RayResult traverseRay(vec4 r0, vec4 dir) {
  ...
  while( rr.color.a<0.95 && r0.w>=0.0 ) {
    ...
    float l = texture3D(surfaceNormals,r0.xyz).a;
    sample = mix(sample, contribution*gradientColor, l);
    sample.a *= (retainedOpacity + boundaryOpacity*pow(l,opacityFactor));
    
    // front-to-back compositing    
    sample.rgb *= sample.a; // opacity-weighted color or associated color        
    rr.color += sample*(1.0-rr.color.a);
