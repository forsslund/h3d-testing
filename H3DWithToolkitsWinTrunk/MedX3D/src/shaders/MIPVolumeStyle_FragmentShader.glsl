...
//
//
/// \file MIPVolumeStyle_FragmentShader.glsl
/// \brief shader file for MIPVolumeStyle, not used by default.
///
//
//
//////////////////////////////////////////////////////////////////////////////
...
RayResult traverseRay(vec4 r0, vec4 dir) {
  ...
  while( r0.w>=0.0 ) {
    // obtain sample value
    float tmp = texture3D(voxels, r0.xyz).r;
  ...
  // look up in transfer function
  rr.color = texture2D(transferFunction,vec2(val,0.5));
  
  // position
  rr.zpoint = vec4(pos,1.0);
