...
//
//
/// \file SilhouetteEnhancementVolumeStyle_FragmentShader.glsl
/// \brief shader file for SilhouetteEnhancementVolumeStyle, not used by
/// default.
///
//
//
...
// UNIFORMS - SilhouetteEnhancementVolumeStyle
uniform float contribution;
uniform sampler3D surfaceNormals;
uniform sampler2D transferFunction;
uniform float silhouetteFactor;
...
RayResult traverseRay(vec4 r0, vec4 dir) {
  ...
  while( rr.color.a<0.95 && r0.w>=0.0 ) {
    ...
    sample.a *= pow(1.0-abs(dot(normal,viewv)),silhouetteFactor);
