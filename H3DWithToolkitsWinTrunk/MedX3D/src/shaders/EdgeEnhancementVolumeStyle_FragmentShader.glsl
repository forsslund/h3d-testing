...
//
//
/// \file EdgeEnhancementVolumeStyle_FragmentShader.glsl
/// \brief shader file for EdgeEnhancementVolumeStyle, not used by default.
///
//
//
//////////////////////////////////////////////////////////////////////////////
...
// UNIFORMS - EdgeEnhancementVolumeStyle
...
RayResult traverseRay(vec4 r0, vec4 dir) {
  ...
  while( rr.color.a<0.95 && r0.w>=0.0 ) {
    ...
    // edge enhancement
    vec4 n = texture3D(surfaceNormals,r0.xyz);
    if( n.a > 0.1 ) {
      vec3 normal = normalize(2.0*n.xyz-1.0);
      float cosa = 1.0-abs( dot(normal,viewv) );
      float contrib = contribution*cosa*step(cosgt,cosa);
      sample = mix(sample,edgeColor,contrib);
      //sample.a += contrib*edgeColor.a;
    }
