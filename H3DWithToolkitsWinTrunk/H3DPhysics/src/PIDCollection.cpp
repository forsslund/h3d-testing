...
PIDCollection::~PIDCollection() {
  if ( engine_thread ) {
	  // remove callback for the updatPhysics function 
	  engine_thread->removeAsynchronousCallback ( physicsCallbackId );
		engine_thread= NULL;
  }
}
...
void PIDCollection::initialize ( PhysicsEngineThread& pt ) {
  ...
  if ( rbc->getValue() ) {
    ...
    physicsCallbackId= engine_thread->asynchronousCallback ( PIDCollection::updatePhysics, this );
  }
}
...
void PIDCollection::traverseSG( TraverseInfo &ti ) {
  ...
  if( pt ) {
    ...
    if( pt == engine_thread ) {
		  ...
		  for( MFJointPIDNode::const_iterator i = pids->begin(); i != pids->end(); i++ ) {
...
H3DUtil::PeriodicThread::CallbackCode PIDCollection::updatePhysics ( void* data ) {
  PIDCollection* pid= static_cast<PIDCollection*>(data);
    ...
    for( vector<H3DPIDNode*>::iterator i = pid->rt_pids.begin(); i != pid->rt_pids.end(); i++ ) {
		  H3DPIDNode* ptr= *i;
			if( ptr ){
...
void PIDCollection::UpdatePIDProperties::update () {
  ...
  for( size_t i = 0; i < _pids.size(); i++ ) {
    o->rt_pids[i]= static_cast < H3DPIDNode* > ( _pids[i] );
  }
