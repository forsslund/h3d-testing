...
void BulletSingleAxisHingeJoint::getParameters ( PhysicsEngineParameters::JointParameters& jointParameters )
{
  ...
  btScalar dotProduct= projBodyA_Y.dot( bodyB_Y );
  dotProduct= std::max ( -1.0f, std::min ( 1.0f, dotProduct ) );
  btScalar angle= acos ( dotProduct );

  // Get the correct sign depending on the direction of rotation
  btVector3 crossProd= projBodyA_Y.cross( bodyB_Y );
  crossProd.normalize();
  if ( crossProd.dot( bodyB_X ) < 0 ) {
    angle= -angle;
  }
  //********************************************************************************//
...
BulletSoftBodyLinearJoint::BulletSoftBodyLinearJoint ( JointParameters& jointParameters ) 
  : BulletJoint ( jointParameters ),
  softBody1 ( NULL ), softBody2 ( NULL ), index ( -1 )
{
...
void BulletSoftBodyLinearJoint::remove () {
  /// \todo Regulary check if bullet have implemented a removeJoint feature.
  /// Since the code below will cause a memory leak because bullet can not release data
  /// allocated in appendLinearJoint and we can not do it either because it was allocated
  /// in a different library (or could be if we are not linking statically).
  if ( softBody1 && index >= 0 ) {
    for ( int i= index; 
      i+1 < softBody1->m_joints.size(); ++i ) {
        softBody1->m_joints[i]= softBody1->m_joints[i+1];
    }
    softBody1->m_joints.resize ( softBody1->m_joints.size()-1 );
    index= -1;
  }
}
...
void BulletSoftBodyAngularJoint::remove () {
  /// \todo Regulary check if bullet have implemented a removeJoint feature.
  /// Since the code below will cause a memory leak because bullet can not release data
  /// allocated in appendAngularJoint and we can not do it either because it was allocated
  /// in a different library (or could be if we are not linking statically).
  if ( softBody1 && index >= 0 ) {
    for ( int i= index; 
      i+1 < softBody1->m_joints.size(); ++i ) {
