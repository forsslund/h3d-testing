...
namespace {
  static PyMethodDef H3DPhysics_PythonMethods[] = {
      {"TrackedMField_insertTracked", TrackedMField_insertTracked, 0},
      {"TrackedMField_updateTracked", TrackedMField_updateTracked, 0},
      {"TrackedMField_eraseTracked",  TrackedMField_eraseTracked, 0},
...
void H3DPhysicsPythonInterface::initModule () {
  ...
  if ( !Py_IsInitialized() ) {
    Py_Initialize();  
    /// \todo Transfer command line args
    //if( argv )
