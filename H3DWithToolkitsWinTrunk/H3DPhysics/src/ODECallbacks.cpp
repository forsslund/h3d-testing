...
PeriodicThread::CallbackCode ODECallbacks::doSimulationSteps(void *data) {
  ...
  if( world_params.getEnabled() ) {
    ...
    for( map< dGeomID, dGeomID >::iterator i = ode_data->tri_meshes.begin();
      ...
      if( index == 0 ) {
        /// \todo why is this done?
        dReal* real_t = dGeomTriMeshGetLastTransform( i->second );
      }
    }
  ...
  if ( dGeomIsSpace(o1) || dGeomIsSpace(o2) ) {
      ...
      if( n > 0 ) {
        for( vector< string >::iterator i = p.applied_parameters.begin();
          ...
          // TODO: we do not have fdir in CollisionCollection. Why is this?
          // This mode is useless then
          //else if( (*i) == "USER_FRICTION" ) mode |= dContactFDir1;
  ...
  if( params->haveMass() || params->haveMassDensityModel() ||
    ...
    if( !n ) {
      ...
      if (Box *b = dynamic_cast< Box* >( n ) ) {
        ...
        // TODO: take the position of the bounding box into consideration
        dMassSetBoxTotal( &ode_mass, params->getMass(), size.x, size.y, size.z);
      } else {
...
H3DUtil::PeriodicThread::CallbackCode ODECallbacks::setCollidableParameters(void *data) {
  ...
  dGeomID geomID = dGeomTransformGetGeom( ( dGeomID )params->getCollidableId() );
  ...
  dGeomSetPosition( geomID, 
                   translation.x, 
                   translation.y, 
                   translation.z );
  dGeomSetRotation( geomID, R);
  
  if( params->getEnabled() )
    dGeomEnable( ( dGeomID )params->getCollidableId() );
  else
    dGeomDisable( ( dGeomID )params->getCollidableId() );
...
H3DUtil::PeriodicThread::CallbackCode ODECallbacks::addCollidable(void *data) {
  ...
  ODECallbacks::setCollidableParameters( params );
  return PeriodicThread::CALLBACK_DONE;
}
...
void ODECallbacks::addOffset( OffsetParameters * params ) {
  ...
  dGeomID transform = dCreateGeomTransform( (dSpaceID)params->getParentSpaceId() );
  dGeomTransformSetCleanup( transform, 0 );
  dGeomTransformSetInfo( transform, 1 );
  params->setCollidableId( ( H3DCollidableId )transform );
  dGeomTransformSetGeom( transform, (dGeomID) params->collidable );
  
  // Move tri_mesh pointer so that only the currently most parent
  // collidable-id points at the tri_mesh
  if( ode_data->tri_meshes.count( (dGeomID) params->collidable ) != 0 ){
    ode_data->tri_meshes[transform] = ode_data->tri_meshes[(dGeomID) params->collidable];
    ode_data->tri_meshes.erase((dGeomID) params->collidable);
  }
}
...
void ODECallbacks::addShape( ShapeParameters * params ) {
  ...
  if ( Box *b = dynamic_cast< Box * >( params->getShape() )) {
    Vec3f size = b->size->getValue();
    temp_geom = dCreateBox(0, size.x, size.y, size.z);
  } else if ( Sphere *sph = dynamic_cast< Sphere * >( params->getShape() )) {
    dReal radius = sph->radius->getValue();
    temp_geom = dCreateSphere(0, radius);
    ...
    temp_geom = dCreateGeomTransform( 0 );
    dGeomTransformSetCleanup( temp_geom, 1 );
    dGeomTransformSetInfo( temp_geom, 1 );
    dGeomID ode_cyl = dCreateCylinder(0, radius, height);
    dMatrix3 R;
    dRFromAxisAndAngle(R, 1, 0, 0, Constants::pi/2 );  
    dGeomSetRotation( ode_cyl, R );
    dGeomTransformSetGeom( temp_geom, ode_cyl );
  } else {
    ...
    temp_geom = dCreateTriMesh( 0, //ode_data->space_id,
                                tri_mesh_id, NULL, NULL, NULL );
    ode_data->tri_meshes[transform] = temp_geom;
  }  
  params->setCollidableId( ( H3DCollidableId )transform );
  dGeomTransformSetGeom( transform, temp_geom );
}
