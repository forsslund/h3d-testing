...
void H3DBodyInteractorNode::SFH3DBodyNode::onAdd( Node *n ) {
  ...
  /* REFER to TODO in traverseSG
  RigidBody *r = static_cast< RigidBody * >( n );
  H3DJointNode *j = static_cast< H3DJointNode * >( getOwner() );
  if ( j->isInitialized() && !r->isInitialized() )
  r->initializeRigidBody( j->engine_thread );
  */
}
