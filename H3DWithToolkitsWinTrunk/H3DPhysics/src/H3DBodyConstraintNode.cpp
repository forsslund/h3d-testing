...
void H3DBodyConstraintNode::traverseSG( TraverseInfo &ti ) {
  ...
  if( pt ) {
    ...
    if( pt == engine_thread ) {
      ...
      /* TODO: handling of unitialized rigid bodies in joint.
      The thing is, if the body is initialized via joint it should not
      be necessarily deleted once dismounted from joint. Yet if it is
      a stand alone body, then it will never be deleted.
      if( ( body1->getValue() != NULL ) && !body1->getValue()->isInitialized() )
      body1->getValue()->initializeRigidBody( pt );
      if( ( body2->getValue() != NULL ) && !body2->getValue()->isInitialized() )
      body2->getValue()->initializeRigidBody( pt );
      */
