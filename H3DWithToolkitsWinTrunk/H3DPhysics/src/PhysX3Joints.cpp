...
#include <H3D/H3DPhysics/PhysX3BodyConstraintOptions.h>
...
void PhysX3BallJoint::setParameters ( ConstraintParameters& jointParameters ) {
  ...
  if( params->haveEngineOptions() ){
    if (PhysX3Joint6DOFLimitParameters* options =
      dynamic_cast<PhysX3Joint6DOFLimitParameters*>(params->getEngineOptions()) ){
      ...
      if( options->haveContactDistance() ){
        ...
        limit_contact_distance.y *= ( H3DUtil::Constants::pi /180.0);
      }
      ...
      if( options->haveProjectionTolerance() ){
        ...
        Vec2f tolerance = options->getProjectionTolerance();
        if( tolerance.x > 0.0 && tolerance.y > 0.0 ){
          s_joint->setProjectionLinearTolerance( PxReal( tolerance.x ) );
          s_joint->setConstraintFlag(PxConstraintFlag::ePROJECTION, true);
        }
        else{
          s_joint->setConstraintFlag(PxConstraintFlag::ePROJECTION, false);
        }
...
void PhysX3SliderJoint::setParameters ( ConstraintParameters& jointParameters ) {
  ...
  if ( params->haveAxis() ) {
    ...
    // TODO: This is not efficient. In case there is have_offset_1 or have_offset_2,
    // the local frame point is overwritten.
  ...
  if( params->haveEngineOptions() ){
    ...
    if (PhysX3SliderJointParameters* options =
      ...
      if( options->haveBody2ForceScale() ){
        body2_slider_froce_scale = options->getBody2ForceScale();
      }
...
void PhysX3SliderJoint::update ()
{
  ...
  if( actor1 ) {
    ...
    static_cast< PxRigidDynamic * >(actor1)->addForce( globalFrame.q.rotate( -the_force*body2_slider_froce_scale ), slider_force_mode );
  }
  if( actor2 ) {
    PxTransform localFrame = joint->getLocalPose( PxJointActorIndex::eACTOR1 );
    PxTransform globalFrame= localFrame;
...
void PhysX3DistanceJoint::setParameters ( ConstraintParameters& jointParameters ) {
  ...
  if ( params->haveMinDistance() ) {
    distanceJoint->setMinDistance( params->getMinDistance() );
  }
...
void PhysX3DistanceJoint::getParameters ( ConstraintParameters& jointParameters ) {
  ...
  if ( params->haveDistance() ) {
    ...
    //params->setDistance ( distanceJoint->getDistance() );
  }
...
PhysX36DoFJoint ( jointParameters ),
  l_limit( PxJointLinearLimit( 0.1, PxSpring( 100, 0 ) ) ),
  c_limit( PxJointLimitCone( 0.1, 0.1, PxSpring( 100, 0 ) ) ),
  t_limit( PxJointAngularLimitPair( 0.0, 1.8, PxSpring( 100, 0 ) ) ) {
