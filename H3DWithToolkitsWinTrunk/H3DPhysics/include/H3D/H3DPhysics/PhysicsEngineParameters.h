  ...
  namespace PhysicsEngineParameters {
    ...
    struct H3DPHYS_API RigidBodyParameters {
      ...
      inline bool haveDebug() {
        return (update_bit_mask & DEBUG) != 0;
      }
      ...
      static const unsigned int DEBUG = 0x20000000;
      static const unsigned int GRAPHICS_FRAME_FORCE = 0x40000000;
      static const unsigned int GRAPHICS_FRAME_TORQUE = 0x80000000;

      /// Bitmask for which parameters that are set.
      /// \todo This should be fixed if we don't want to start using c++11 features.
      /// long long is a c++11 feature and if this should be fixed to support previous
      /// versions then update_bit_mask have to be split into two 32 bit types.
