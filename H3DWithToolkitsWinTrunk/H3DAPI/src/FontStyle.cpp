...
namespace FontStyleInternals {
  ...
  class FontStyleMap : public std::map< std::string, FTFont* > {
  public:
  ...
  #ifdef HAVE_FONTCONFIG
  std::string FC_GetFontByName(const char* font_name, bool bold, bool italic) {
  ...
  FTFont* getFontByName(const std::string& font_name,
                        bool bold,
                        bool italic,
    ...
    std::string full_font_name = font_name;
    if(bold) {
    ...
    #ifdef H3D_WINDOWS
    std::string font_display_name, font_file_name;
    if(!GetFontFile(full_font_name.c_str(), font_display_name, font_file_name)) {
      return NULL;
    }

    std::string path = std::string(getenv("windir")) + "\\fonts\\";
...
void FontStyle::buildFonts() {
  ...
  const std::string& render_type = renderType->getValue();
  ...
  const std::string& font_style = style->getValue();
  bool italic = false;
  bool bold = false;
  ...
  for(MFString::const_iterator n = family->begin(); n != family->end(); ++n) {
...
X3DFontStyleNode::Justification FontStyle::getMajorJustification() {
  if(justify->size() == 0)
    ...
    const std::string& s = justify->getValueByIndex(0);
    if(s == "") return BEGIN;
...
X3DFontStyleNode::Justification FontStyle::getMinorJustification() {
  if(justify->size() < 2)
    ...
    const std::string& s = justify->getValueByIndex(1);
    if(s == "FIRST") return FIRST;
    if(s == "BEGIN") return BEGIN;
    if(s == "MIDDLE") return MIDDLE;
    if(s == "END") return END;
...
void FontStyle::renderHorizontalLine(const std::string& text) {
