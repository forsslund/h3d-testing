        ...
        if(model[i].hasOwnProperty('children'))
          ...
          for(var j = 0; j < model[i].testcases.length; j++) {
            ...
            if(testcase.result_type=='performance') {
              if($.inArray(testcase.build_name, display_options.servers.available) < 0) {
                display_options.servers.available.push(testcase.build_name);
              }
              if(display_options.servers.selected.length == 0) {
                display_options.servers.selected.push(testcase.build_name);
              }
              ...
              for(var k = 0; k < testcase.profiler_data.data.length; ++k) {
                if($.inArray(testcase.profiler_data.data[k].build_name, display_options.servers.available) < 0) {
                  display_options.servers.available.push(testcase.profiler_data.data[k].build_name);
                }
              }
...
function refreshDisplayOptions(model) {
  ...
  for(var i = 0; i < display_options.servers.available.length; i++) {
    ...
    $('#Option_Servers').append(server_list[display_options.servers.available[i]].build + "(" + server_list[display_options.servers.available[i]].hardware + ")" + "<br/>");
        ...
        for(var i = 0; i < arr.children.length; ++i) {
          ...
          if(level < 3) {
            for(var j = 0; j < arr.children[i].data.length; ++j) {
              if(arr.children[i].data[j].build_name == server) {           
            ...
            if(dataset.length > 0)    
              datasets.push({label: arr.children[i].id, stack: level + (s*4), data: dataset, build_name: server_list[server].build, hardware_name : server_list[server].build, prof_data: arr.children[i]});
          }
...
function SetServer(server_index) {
  ...
  display_options.servers.available = [server_index];
  display_options.servers.selected = [server_index];
