      ...
      for result in step.results:
        if type(result).__name__ == 'ErrorResult':
          ...
          # If no previous run for this server then set new_failure to Y
            ...
            new_failure = 'Y'
          curs.execute("INSERT INTO error_results (test_run_id, file_id, case_id, step_id, stdout, stderr, new_failure, error_type) VALUES (%d, %d, %d, %d, '%s', '%s', '%s', '%s')" % (self.test_run_id, testfile_id, testcase_id, teststep_id, self.db.escape_string(result.stdout), self.db.escape_string(result.stderr), new_failure, result.error_type))
          ...
          if result.success:
            ...
            if os.path.exists(result.baseline_path):
              ...
              f.close()
              curs.execute("INSERT INTO console_results (test_run_id, file_id, case_id, step_id, success, output, baseline, diff, new_failure, error_type) VALUES (%d, %d, %d, %d, 'N', '%s', '%s', '%s', '%s')" % (self.test_run_id, testfile_id, testcase_id, teststep_id, self.db.escape_string(output_string), self.db.escape_string(baseline_string), self.db.escape_string(result.diff), new_failure, result.error_type))
            else:
              curs.execute("INSERT INTO console_results (test_run_id, file_id, case_id, step_id, success, output, baseline, diff, new_failure, error_type) VALUES (%d, %d, %d, %d, 'N', '%s', NULL, NULL, '%s')" % (self.test_run_id, testfile_id, testcase_id, teststep_id, self.db.escape_string(output_string), new_failure, result.error_type))
