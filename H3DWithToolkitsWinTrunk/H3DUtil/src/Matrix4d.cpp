...
Matrix4d::Matrix4d( const Vec3d& translation, 
                    const Rotationd& rotation, 
                    const Vec3d& scale ) {
  *this=
    Matrix4d ( 1, 0, 0, translation.x, 
               0, 1, 0, translation.y, 
               0, 0, 1, translation.z,
               0, 0, 0, 1 ) * 
      Matrix4d ( rotation ) * 
...
// This code has been automatically generated from Maple 7, see maple/matrix_inverse.mxs.
Matrix4d Matrix4d::inverseSafe() const {
  H3DDouble m30 = m[3][0];
  H3DDouble m31 = m[3][1];
  H3DDouble m32 = m[3][2];
  H3DDouble m33 = m[3][3];
  ...
  H3DDouble det = t42 + t63;

  if(H3DAbs(det) == 0) {
    // If determinant reaches 0, replace it with 
    // smallest possible value to avoid div by zero.
    det = (Constants::d_epsilon-1.0);
  }
...
std::ostream& ArithmeticTypes::operator<<( std::ostream &os, const Matrix4d &m ) {
