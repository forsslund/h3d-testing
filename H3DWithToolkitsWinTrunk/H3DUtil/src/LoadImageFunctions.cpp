...
H3DUTIL_API bool H3DUtil::saveOpenEXRImage( const string &url,
  ...
  try {
    ...
    if ( offsets[0] != -1 ) header.channels().insert ("R", Imf::Channel (Imf::FLOAT));
    if ( offsets[1] != -1 ) header.channels().insert ("G", Imf::Channel (Imf::FLOAT));
    if ( offsets[2] != -1 ) header.channels().insert ("B", Imf::Channel (Imf::FLOAT));
    if ( offsets[3] != -1 ) header.channels().insert ("A", Imf::Channel (Imf::FLOAT));
