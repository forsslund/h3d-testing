...
namespace H3DUtil {
  ...
  /// \ingroup H3DUtilMath
  /// Returns value a, clamped to the range between min and max.
  template< class A >
  inline A H3DClamp( const A &a, const A& min, const A& max) {
    return H3DMax(min, H3DMin(a, max));
  }
